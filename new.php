...
<head>
  ...
  <script src="tinymce4/js/tinymce/tinymce.min.js"> 
    ...
    <script> tinymce.init({
		...
		font_format: 'Calibri,Ubuntu,Arial',
...
plugins: [
              ...
              "insertdatetime media nonbreaking save table code contextmenu directionality",
			...
			<a id="menutoggle" ><i class="glyphicon glyphicon-align-justify"></i> </a>
             <nav class="xs-menu displaynone">
					<ul>
						...
						<li class="active"><a href="new.php">Upload</a></li>
...
<div class="container" style="background: rgba(580, 400, 400, 0.90); margin-top:-8px">
	 ...
	 ?>
      ...
      <div class="row" border=1px >
       ...
       <div class="col-lg-8 col-md-12 col-sm-12 col-xs-12"  style="border: 1px solid #666666; margin-left:15px; 
         margin-right: -15px; border-radius: 7px;  margin-top: 5px;">
		 ...
		 if(isset($_POST["upload"])){
			 ...
			 $postBy = $_POST["postBy"];
			 $postnews = mysql_query("insert into post(title,news,postBy)values('$title','$news','$postBy')") or die("could not post news".mysql_error());
			 if($postnews)
			 {
				
				 ?>
				 <script>
				 alert("Your post is Succesfully") 
		...
		<form name="uploadNews" method ="POST">
		  <div class="row">
		  ...
		  <input type="hidden" class="form-control" id="postBy" name="postBy" placeholder="Document title" required>
          </div>
         
         
		</div>
		
     <textarea class="form-control col-lg-12 col-md-12 col-sm-12 col-xs-12 textarea" rows="10" id="news"  
     style=" width:100%;height:450px; margin-bottom:10px;" name="news"  required>
     </textarea>
	...
	<div class="row">
	<div class="col-lg-6">
   <input type="submit" value="Upload" class="btn btn-success btn-block tooltip-viewport-bottom" name="upload"  title="Create a new document"/>
   </div>
   <div class="col-lg-6">
  ...
  data = JSON.parse(data);
   ...
   if(role!=="editor" && role!=="Admin" )
   {
	...
	$("#upload").click(function(e){
		e.preventDefault()
	title = $("#title").val();
	news = tinymce.get("rubbish").getContent
	alert("the news"+news)
	
	if (title.length<1 || news.length<1 ){
			alert("Please fill in data to both fields") 
			}else
			{
	
	var formdata={ 
	title : $("#title").val(),
	news : tinymce.get("news").getContent,
	postBy : userid,
	...
	$.ajax({
		url:"http://localhost/NTA/api.php/post",
		type:"POST",
